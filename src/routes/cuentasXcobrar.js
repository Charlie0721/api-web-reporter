...
router.get('/api/consulta-cuentas-por-cobrar-info/almacenes/:userId', async (req, res) => {
...
router.post('/api/informe-cuentas-por-cobrar/:idalmacen/:userId', async (req, res) => {
    ...
    try {
        ...
        if (fecha1 === '' || fecha1 === null || fecha1 === ' ') {
            ...
            if (id == 0) {
                ...
                if (typeof item !== "undefined") {
                    await item.connection.query('SELECT a.idcartera, nit, CONCAT(nombres, " ", nombre2, " ", apellidos, " ", apellido2) AS nomtercero, b.direccion, b.telefono, b.telefono2, a.iddocumento AS iddocument, a.detalle, nommunicipio AS nommunicip, nomdepartamento AS nomdepto, fechadoc, fechacuota, valcuota, debito, credito, a.idtercero, a.idmovcuota, a.idvendedor, i.numero AS numerofact, j.numero AS numeroped, k.numero AS numerodoc, co.docprovee,alm.nomalmacen, NULL valpago, NULL fechapago, NULL detpago, NULL numdocpago, NULL nuevosaldo ' +
                        'FROM cartera a ' +
                        ...
                        'LEFT JOIN pedidos j ON (a.iddocumento = j.idpedido AND a.tipodoc = "PEDIDO") LEFT JOIN documentos k ON (a.iddocumento = k.iddocumento AND a.tipodoc = "DOCUMENTO") ' +
                        ...
                        'WHERE a.fechacuota <= ' + fecha1 + ' AND alm.idempresa = 1 AND ((a.valcuota + a.debito) - a.credito) > 0.00 AND a.tipocartera = 1 ' +
                ...
                if (typeof item !== "undefined") {
                    await item.connection.query('SELECT a.idcartera, nit, CONCAT(nombres, " ", nombre2, " ", apellidos, " ", apellido2) AS nomtercero, b.direccion, b.telefono, b.telefono2, a.iddocumento AS iddocument, a.detalle, nommunicipio AS nommunicip, nomdepartamento AS nomdepto, fechadoc, fechacuota, valcuota, debito, credito, a.idtercero, a.idmovcuota, a.idvendedor, i.numero AS numerofact, j.numero AS numeroped, k.numero AS numerodoc, co.docprovee,alm.nomalmacen, NULL valpago, NULL fechapago, NULL detpago, NULL numdocpago, NULL nuevosaldo ' +
                        ...
                        'LEFT JOIN compras co ON (a.iddocumento = co.idcompra AND a.tipodoc = "COMPRA") ' +
                        'LEFT JOIN almacenes alm ON (a.idalmacen = alm.idalmacen) '+
                        'WHERE a.fechacuota <= ' + fecha1 + ' AND a.idalmacen IN ' + '(' + id + ')' + ' AND ((a.valcuota + a.debito) - a.credito) > 0.00 AND a.tipocartera = 1 ' +
                        'ORDER BY b.nombres, a.idtercero, a.fechacuota, a.iddocumento, a.idcartera ', (err, rows, fields) => {
